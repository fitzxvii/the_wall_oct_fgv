class User < ApplicationRecord
    ...
    # Returns: {:status => true/false, :result => {}, :error => {errors}}
    def self.create_user(params)
        response_data = {:status => false, :result => {}, :error => {}}

        begin
            # Validate user parameters
            validate_parameters = self.validate_user_form(params)

            if validate_parameters[:status]
                # Check email if existing on database
                get_user = query_select_one(["SELECT id FROM users WHERE email = ?", validate_parameters[:result][:email]])

                if !get_user
                    # Encrypt password to MD5
                    validate_parameters[:result][:password] = Digest::MD5.hexdigest(validate_parameters[:result][:password])

                    # Prepare query parameters for insert new user
                    query_params = validate_parameters[:result].values_at(:first_name, :last_name, :email, :password)
                    ...
                    if insert_user_record > 0
                        response_data.merge!({
                            ...
                            :result => {
                                ...
                                :first_name => validate_parameters[:result][:first_name],
                                :last_name  => validate_parameters[:result][:last_name]
                            }
                        })
                    else
                ...
                response_data[:error] = validate_parameters[:error]
            end
        rescue Exception => ex
    ...
    private
        # DOCU: Check and validate user form before inserting new record
        # Triggered by: self.create_user
        # Requires: user_form_params - email, first_name, last_name, password, confirm_password
        # Returns: {:status => true/false, :result => user_form_params, :error => {:email, :first_name, :last_name, :password, :confirm_password}}
        def self.validate_user_form(user_form_params)
            response_data = {:status => false, :result => {}, :error => {}}

            user_form_params.each do |key, value|
                # Check current key. If first_name or last_name, set key to :name to reuse checker for special keys.
                ...
                # Validation for email address. If the email is not in correct format, store the error.
                response_data[:error][:emai] = "Email Address not in correct format." if user_param_key === :email && !(user_form_params[:email] !=~ URI::MailTo::EMAIL_REGEXP)
                ...
                # Validation for confirm_password. If confirm password is not the same as first password, store the error.
